---
description: Bootstrap and manage a Jira ticket work session — create Obsidian context notes, set up git worktrees across repos, check PR status, and handle the full lifecycle (start, return, done, add-repo). Use when starting work on a ticket (e.g. "start SG2-1234", "open ticket PROJ-456"), returning to an in-progress ticket, marking a ticket done, or adding a repo to an existing workspace.
...
5. Open `_context.md` in Obsidian (derive the relative vault path by stripping `~/Documents/Lavakrew/` from `TICKET_PATH`):
   ```bash
...
3. Detect default branch, fetch, create worktree (same as Step 2a.5).
